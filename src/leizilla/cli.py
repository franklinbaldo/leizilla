...
def cmd_wayback_save(
    ente: str = typer.Option("ro", help="Ente federativo (ro, ...)"),
    fonte: str = typer.Option("casacivil", help="Fonte (casacivil, ...)"),
    tipo: Optional[str] = typer.Option(None, help="Filtrar tipo (lei, lc, decreto, ...). None = todos"),
    start: int = typer.Option(1, help="Número inicial do range"),
    end: int = typer.Option(0, help="Número final (0 = usar fim do manifesto)"),
    delay: float = typer.Option(2.0, help="Segundos entre submissões ao Wayback"),
    dry_run: bool = typer.Option(False, "--dry-run", help="Mostrar URLs sem submeter"),
    skip_head_check: bool = typer.Option(False, "--skip-head-check", help="Não verificar se URL existe na fonte"),
) -> None:
    """Submete ao Wayback Machine Save Page Now as URLs da ditel que ainda não estão arquivadas."""
    import time
    ...
    # Monta prefixos CDX únicos das estratégias wayback-cdx desta fonte
    ...
    # Consulta CDX uma vez por prefixo
    ...
    # Para cada estratégia sequential, gera lista de candidatos
    total_submitted = 0
    total_skipped = 0
    total_existing = 0
    ...
    for strat in sequential_strategies:
        ...
        echo(f"\nEstratégia: {templates[0].format(num='N')} [{s_start}–{s_end}] head_check={head_check}")
