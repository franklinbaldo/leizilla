...
timestamp: 2026-09-25T06:45:00+00:00
...
fronts:
  ...
  - id: "ro-coverage-q4-2026"
    ...
    key_results:
      ...
      - id: "kr-ro-backlog-conversion"
        ...
        next_action: "Re-ran `uv run leizilla coverage --ente ro --json` 2026-09-25T05:22Z (read-only, no credentials): S4 for lei is STILL 20 (decreto still 0/307) — unchanged from the frozen baseline. Confirms `s4_estruturadas` counts distinct raw_ids with *any* parsed XML present on the IA parsed item, not rows actually published in the dataset, so item 4/13's reparse (run 36094205340) didn't move this counter (their parsed XML already existed pre-fix, just failed the XSD gate). The real lever for this KR is parsing the untouched decreto backlog (215 items already at S3 with OCR text, 0 at S4) via parse-release.yml's workflow_dispatch path (`tipo=decreto`). NOT dispatched this session: the daily Gemini free-tier quota (~20 req/day/model, see parse-release.yml's cron comment) is already fully allocated to the scheduled 06:00 UTC cron (assembleia+lei+lc, 6+6+6=18) which fires shortly after this reading (05:22Z) — a manual dispatch now would compete for the same per-project daily quota and risk 429s on the scheduled run instead of adding net-new throughput. Next session: dispatch `parse-release.yml` (workflow_dispatch, ente=ro fonte=casacivil tipo=decreto, small limit e.g. 6-10) on a day/time that doesn't overlap the 06:00 UTC scheduled cron, then re-read coverage to confirm S4 growth on decreto specifically."
  ...
  - id: "legal-semantic-integrity"
    ...
    key_results:
      ...
      - id: "kr-release-validation-gated"
        ...
        issues: [118, 201, 229]
        evidence_prs: [192, 193, 198, 206, 213, 216, 218, 221, 222, 228]
  ...
  - id: "release-boundary-validation"
    ...
    issues: [118, 201, 229]
    evidence_prs: [192, 193, 198, 206, 213, 215, 216, 218, 221, 222, 228]
