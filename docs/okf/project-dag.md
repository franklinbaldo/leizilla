...
timestamp: 2026-09-25T07:30:00+00:00
...
fronts:
  ...
  - id: "ro-coverage-q4-2026"
    ...
    key_results:
      ...
      - id: "kr-ro-backlog-conversion"
        ...
        next_action: "New session 2026-09-25T07:2xZ: confirmed via the Actions API that this repo's daily parse-release.yml SCHEDULE has NOT been firing at its nominal 06:00 UTC cron time — the last 10 scheduled runs (event=schedule) landed 10:21-12:03 UTC on their respective days, i.e. GitHub's best-effort scheduler delays this low-traffic repo's cron by 4-6h. At the time of this dispatch (07:23 UTC) no schedule run had fired yet today, so the daily Gemini free-tier quota was still fresh — dispatched parse-release.yml (workflow_dispatch, ente=ro fonte=casacivil tipo=decreto, `limit=2`, run 36107356330) using exactly the 2-request margin the cron's own comment reserves (20/day quota − 18/day cron budget = 2 spare), so it cannot starve today's later scheduled cron even if that fires before this dispatch completes (same `concurrency: group: parse-release` serializes them, cancel-in-progress:false). Run was still in_progress at session end — next session: check run 36107356330's conclusion and re-read `uv run leizilla coverage --ente ro --json` to confirm S4(decreto) moved off 0. If it lands clean, dispatching a second small batch (still respecting the 2/day margin, or on a day when the scheduled cron is confirmed not to fire) is the repeatable lever for this KR."
      - id: "kr-ro-recurring-cycle-health"
        ...
        current: "#114 and #121 closed with evidence (2026-09-24). #136/#140/#141/#149 remain open, root causes confirmed distinct: #136/#140 are the legacy rondonia_crawler.yml Playwright range-scan exceeding the 360min job timeout on high-volume sources (untouched — gated on pipeline-convergence); #141 is discover-harvest.yml hitting Internet Archive upload rate limits; #149 is wayback-save.yml's fixed 2s/URL pacing exceeding the 360min budget. PR #207's scope-split fix is CONFIRMED working for #149: run 36082098747 completed successfully (all 8 matrix jobs, none timed out) — closing #149 once a second clean run (scheduled or dispatched) confirms it wasn't a fluke, per this KR's 2-consecutive-cycles target. #141's discover-harvest.yml (run 36082100347) also confirmed making real, correct progress this session (its `lei` job completed successfully end-to-end: discover 1h31m + harvest 1h24m; `lc` started immediately after) — not a hang. But at ~3h/tipo, 8 tipos serialized (`max-parallel: 1`) means one full manifest-driven cycle can take the better part of a day, which is new information not previously measured; whether that still lands inside the weekly schedule's window is unverified."
        ...
        next_action: "#149: verified once (run 36082098747, a workflow_dispatch, not a scheduled run) — the KR's target specifically needs 2 consecutive SCHEDULED cycles, so close only after wayback-save.yml's own cron (not a manual dispatch) produces a second clean run; no scheduled run of it fired between this session and the previous one, so still open. #141: run 36082100347 re-checked 2026-09-25T05:45Z, ~3h after start — still only 1/8 tipos (`lei`) complete, `lc` mid-discovery, 5 tipos still queued; genuinely progressing, not stuck, but the full-cycle wall-clock question from the previous session remains unanswered. Let it finish (or check a future scheduled run) before deciding whether max-parallel:1 alone is sufficient or whether per-scheduled-run tipo scoping is also needed. #136/#140 close only once rondonia_crawler.yml is deprecated (pipeline-convergence)."
  ...
  - id: "ingestion-resilience"
    ...
    next_action: "Issue #121 (the transient-failure-becomes-permanent-loss defect) is fixed and merged (PR #190). #149's scope-split fix (PR #207) is verified working (wayback-save.yml run 36082098747, all 8 matrix jobs green) but still needs a second SCHEDULED (not dispatched) clean run before closing — see kr-ro-recurring-cycle-health. #141's discover-harvest.yml (run 36082100347) is confirmed genuinely progressing, not stuck, but full-cycle wall-clock at the new serialized pace is unmeasured — see kr-ro-recurring-cycle-health. #136/#140 stay deferred to pipeline-convergence. Minor hygiene fixed this session (not tied to a numbered issue): tests/test_scrape_skip_existing.py's bare `@patch('leizilla.cli.asyncio.run')` mocks left `cmd_scrape`'s inner coroutine uncreated-but-unawaited, producing 'coroutine was never awaited' RuntimeWarnings misattributed to unrelated later tests at GC time; fixed by closing the coroutine in the mock's side_effect. `uv run pytest tests/test_scrape_skip_existing.py -W error::RuntimeWarning` now passes clean."
  ...
  - id: "legal-semantic-integrity"
    ...
    key_results:
      - id: "kr-date-provenance-honest"
        ...
        next_action: "#167 has its own resume precondition (official visual capture of /lei/'s Dados section must succeed again, or a canonical UI fixture must exist) before implementing the frontend disclosure copy — see public-surface-auditability. #229 implemented this session: PR #230 (open) adds `inicio_fontes` (VARCHAR JSON, same {ia_id} shape as `fontes`) to PARQUET_SCHEMA/xml_to_rows, fail-closed on a missing ia-id, documents it in docs/SCHEMA.md §3.1, and surfaces it through LeiRow/EXPORT_COLUMNS/Versoes.svelte as archive.org evidence links next to each version's inicio_tipo label. `uv run leizilla dev check` (918 passed) + mypy + web `vitest run` (53 passed) + `astro build` all green locally; CI (lint.yml run 36107734133) in progress at session end — next session: confirm CI green and merge, then close #229."
  ...
  - id: "public-surface-auditability"
    ...
    issues: [167, 229]
    next_action: "kr-public-responsive-audit and kr-law-page-regression-tests are met. Only kr-public-semantic-legibility (#167, #229) remains, gated on its own external observability precondition — see that KR."
    key_results:
      - id: "kr-public-semantic-legibility"
        ...
        issues: [167, 229]
        next_action: "Do not implement #167's UI change yet. Re-check which of its 3 criteria still apply given data_ato is already the live column name. Resume path: either resolve browser-read-cors-integrity (proxy or fixed IA CORS) so a real `/lei/?id=...` capture becomes possible, or build a canonical UI fixture (a vitest+jsdom render of `Dados.svelte`/`Versoes.svelte` against representative sample rows, following the existing model.test.ts pattern) — only then add whatever disclosure copy is still warranted. Consider sequencing #229 (surface the <inicio> evidence link) before or alongside #167's copy change — showing the actual fonte may be a stronger fix than wording alone."
