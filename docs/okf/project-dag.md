...
fronts:
  ...
  - id: "legal-semantic-integrity"
    ...
    key_results:
      ...
      - id: "kr-temporal-version-valid"
        ...
        current: "Issue #120 remains open. A dedicated implementation now sorts multiple versions by effective date and rejects multi-version timelines whose dates cannot be resolved; project gates have not yet run on that change."
        target: "0 known overlapping-current states caused by missing/out-of-order dates; regression coverage present."
        issues: [120]
        next_action: "Run the declared project gates on the temporal fix and review it; only mark the KR met after reproducible green evidence is merged."
  ...
  - id: "temporal-version-integrity"
    ...
    origin: "Issue #120."
    issues: [120]
    next_action: "A regression and fail-closed ordering implementation exist on a dedicated branch; run project gates and review before merge, then close #120 only with reproducible evidence."
