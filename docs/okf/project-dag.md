...
fronts:
  ...
  - id: "dataset-release-integrity"
    ...
    next_action: "Item 4 is FULLY resolved as of this session: PR #198, #203, #204, #210, #213, #216, #218 all merged — six distinct bugs fixed in its reparse/validation chain (existence-check, prompt, OCR-fetch fallback, Gemini cache-quota, over-escaped-quote JSON, Gemini thinking-token truncation, roman-numeral path segments). Verified live: a real, non-dry-run dispatch after #218 merged (run 36088349740) shows `consolidate` now skips only 1/20 files — item 13 — where it previously skipped 2/20; item 4 is clean. Only two blockers remain for `-latest`: (1) item 13, a separate external IA-side gap (no OCR derivative under either filename, no code fix available), and (2) an unexplained, NOT-yet-investigated gap — the same run's candidate dataset has 171 rows (19/20 valid items), still below the 199-row floor from the currently published legacy `leizilla-dataset-ro-v0` item (`fetch_published_dataset_row_count` falls back to it since `-latest` was never published — PR #193's guard). 199 > 171 means an EARLIER `--version 0` scheduled release had more valid rows than today's corpus produces — i.e. more items were valid then than the 19/20 available now. Whether that's normal week-to-week corpus churn (`--skip-existing` overwriting a previously-good item with a worse reparse) or actual regression/data-loss on IA's parsed area has not been investigated; this is now the single concrete next step, along with item 13's external status, before `-latest` can publish. Do not force the floor guard open to work around either."
  ...
  - id: "legal-semantic-integrity"
    ...
    key_results:
      ...
      - id: "kr-release-validation-gated"
        ...
        next_action: "None on #118 (closed). #195 (urn_lex prefix-corruption gate) closed via merged PR #206: `xml_to_rows` now rejects a present-but-corrupted `urn-lex` that doesn't start with `urn:lex:br` (e.g. the literal string \"null\"), without regressing the #127/#191 lei_id-fallback for grammar-level mismatches. #201 is down to 1/20 rejected items after PR #218 merged and was verified live (run 36088349740): item 4 now clears the gate; only item 13 (literal `urn-lex=\"null\"`, external IA-side gap, no code fix) still fails. See dataset-release-integrity for the remaining, unexplained 171-vs-199-row floor gap that still blocks `-latest`."
  ...
  - id: "release-boundary-validation"
    ...
    next_action: "See legal-semantic-integrity's kr-release-validation-gated for what's merged (PRs #192, #193, #198, #206 — #195 closed). #201: item 4 is fully resolved — PRs #204, #210, #213, #216, #218 (all merged) each fixed a distinct bug, verified live (run 36088349740: `consolidate` now skips only item 13, not item 4). See dataset-release-integrity for the remaining external IA-side blocker (item 13) and the unexplained 171-vs-199-row floor gap that still blocks `-latest`."
