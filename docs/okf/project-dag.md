...
fronts:
  ...
  - id: "dependency-security-hygiene"
    ...
    next_action: "GitHub's own push-output alert count dropped 24 -> 8 (after PR #237) -> 2 (after PR #240), and 2 moderate is exactly what `npm audit` still reports (vitest/@vitest/mocker path-traversal, GHSA-82fw-gwwq-j7x9) — strong evidence PR #240's uv-group bump (anyio/pytest/pygments/soupsieve) closed the entire remaining Python-side gap, not just a partial slice. Not independently confirmed via the Dependabot alerts API itself (no tool in this session's environment could read `/repos/.../dependabot/alerts` — 403, insufficient token scope), so this is push-output evidence, not an API-verified zero; a future session with a `security_events`-scoped token (or a maintainer via the web UI) should confirm directly. Remaining 2 moderate findings are the same known vitest 3->5 major-bump case from PR #237 — deliberately not fixed (dev-only tooling, not shipped to the built site, risks breaking the test API surface) — deserves its own verified PR. GitHub Actions workflow pin versions remain unchecked as a residual audit gap."
---
