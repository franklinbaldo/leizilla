...
fronts:
  ...
  - id: "legal-semantic-integrity"
    ...
    key_results:
      ...
      - id: "kr-release-validation-gated"
        ...
        next_action: "None on #118 (closed). #195 (urn_lex prefix-corruption gate) closed via merged PR #206: `xml_to_rows` now rejects a present-but-corrupted `urn-lex` that doesn't start with `urn:lex:br` (e.g. the literal string \"null\"), without regressing the #127/#191 lei_id-fallback for grammar-level mismatches. #201 is the gate correctly doing its job: it rejects 2/20 already-published `ro` items on real XSD violations (roman-numeral path segments, literal `urn-lex=\"null\"`) — see dataset-release-integrity for the full reparse/root-cause chain (PR #204/#210/#213/#216 fixed four distinct bugs, all merged; item 13 is an external IA-side gap; #216's production verification dispatch was still running at session end)."
  ...
  - id: "release-boundary-validation"
    ...
    next_action: "See legal-semantic-integrity's kr-release-validation-gated for what's merged (PRs #192, #193, #198, #206 — #195 closed). #201: PRs #204, #210, #213, #216 (all merged) each fixed a distinct bug blocking item 4's reparse — #216 (this session) was the Gemini 'thinking'-tokens-truncate-before-max_tokens bug found via PR #215's diagnostics (run 36086040197). See dataset-release-integrity for the full chain, the pending verification re-dispatch (run 36086998353, not observed to completion this session), and the remaining external IA-side blocker (item 13)."
