...
fronts:
  ...
  - id: "dataset-release-integrity"
    ...
    evidence_prs: [198, 204, 210, 213]
    next_action: "Chain of fixes, in order: PR #198 (503-vs-404 existence-check), #203 (parser prompt fix), #204 (OCR bare-filename fallback for item 4) and #210 (Gemini `cache_control` quota fix) are all merged — item 4's LLM call now completes instead of failing on OCR resolution or 429ing. Re-dispatching `parse-release.yml` for item 4 after #210 (run 36078510721) then hit a third, distinct bug: `_extract_json` failed on a response wrapped in a ` ```json ` fence whose `xml` field value contained double-backslash-escaped quotes (invalid JSON string escaping) — `json.loads`/`raw_decode` both terminate the string early on the extra backslash and the rest of the payload no longer parses. PR #213 (open, not yet merged) fixes this: `_extract_json` retries once with the extra backslash collapsed before giving up, with regression tests reproducing the exact fenced/escaped shape. Once #213 merges, item 4 needs one more `parse-release.yml` re-dispatch with real credentials to confirm end-to-end. Item 13 remains a separate external IA-side blocker (no OCR derivative under either filename) with no code fix available. `-latest` stays unpublished and #196 stays open until both items 4 and 13 are resolved (or a deliberate one-time floor exception is decided) — do not force the floor guard open to work around this."
  ...
  - id: "release-boundary-validation"
    ...
    issues: [118, 201]
    evidence_prs: [192, 193, 198, 206, 213]
    next_action: "See legal-semantic-integrity's kr-release-validation-gated for what's merged (PRs #192, #193, #198, #206 — #195 closed). #201: PR #204 (merged) fixes the parser's OCR-fetch fallback for one of the 2 gate-rejected items; PR #210 (merged) fixes a second, independently-discovered bug (Gemini cache_control quota) that was blocking the reparse itself; PR #213 (open) fixes a third, distinct issue found once the LLM call started completing (malformed/over-escaped JSON in the response). Once #213 merges, re-dispatch `parse-release.yml` for item 4 with real credentials to confirm end-to-end; see dataset-release-integrity for the full chain and the remaining external IA-side blocker (item 13)."
