...
fronts:
  ...
  - id: "ro-coverage-q4-2026"
    ...
    key_results:
      ...
      - id: "kr-ro-recurring-cycle-health"
        ...
        current: "0 qualifying consecutive healthy cycles. The 2026-09-20 legacy casacivil-lei run timed out after about 308 items with only 3 successes; 176/305 failures were masked as Falha [?]. Issues #136/#140 recur weekly, while #114/#141/#149 remain unresolved trackers."
        ...
        evidence_prs: [187]
        next_action: "Land the minimal upload-error-detail fix from #187 without its stale IMPLEMENTATION merge conflict, inspect the next real crawler log for the underlying IA/fetch failure, then fix that cause before counting a cycle as healthy."
  ...
  - id: "ingestion-resilience"
    ...
    evidence_prs: [187]
    next_action: "First restore diagnostic observability for #136/#140 with the minimal #187 code/test slice; use the next scheduled evidence to classify the dominant failure, then repair retry/requeue semantics from #121 without treating infrastructure failure as domain evidence."
