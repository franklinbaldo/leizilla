...
fronts:
  ...
  - id: "public-surface-auditability"
    ...
    issues: [167, 229]
    next_action: "kr-public-responsive-audit and kr-law-page-regression-tests are met. #229 closed this session (PR #230). Only kr-public-semantic-legibility's remaining #167 slice stays open, gated on its own external observability precondition — see that KR."
    key_results:
      - id: "kr-public-semantic-legibility"
        ...
        current: "The schema-level conflation this KR depended on (date-provenance, #157) is resolved and closed. Issue #167's resume precondition (official visual-capture must load the published Parquet and open a norm's Dados section, or a canonical UI fixture must exist) is now explained, not just unmet: see the new browser-read-cors-integrity node — archive.org never sends CORS for .parquet downloads (confirmed live this session directly against archive.org production, not just a local dev server), so read_parquet() cannot succeed from any real browser today, which is why the prior visual-capture attempt (run 33956952230) timed out. Also confirmed this session: EXPORT_COLUMNS/model.ts already use `data_ato`, not the legacy `data_publicacao` name #167 was filed against, so criterion 1 of #167 (legacy field name in JSON/CSV downloads) may already be moot; only criterion 3 (inicio_tipo='data-publicacao' rendered categorically as 'vigência desde a publicação') still looks live. PR #228 makes criterion 3's label materially more honest at the mechanism level (data-publicacao can no longer be a silent ETL default — see legal-semantic-integrity's kr-date-provenance-honest). Issue #229 CLOSED this session via PR #230 (merged): Versoes.svelte now renders the `<inicio>`'s `<fonte>` evidence as archive.org links next to each version's inicio_tipo label — the UI shows the *evidence* behind the label, not just the trusted word, for every version that has one. This is exactly the 'showing the actual fonte may be a stronger fix than wording alone' sequencing this KR's previous next_action proposed. What remains is #167's own copy change (whether the inicio_tipo label text itself needs rewording) — still gated on the same browser-read-cors-integrity precondition, since that change still needs a real capture (or fixture) of /lei/'s Dados section to verify against representative data."
        target: "0 known misleading labels; legacy fields are explained where still published."
        blockers: ["browser-read-cors-integrity — the Dados section cannot be observed against real, populated data from any real browser until the CORS gap is fixed (proxy or IA-side), or a canonical UI fixture is built."]
        issues: [167]
        evidence_prs: [230]
        next_action: "Do not implement #167's UI copy change yet. Re-check which of its 3 criteria still apply given data_ato is already the live column name and #229's evidence links are now live. New session 2026-09-25T08:3xZ: queried the live published Parquet directly (server-side, no browser needed for this check) — 0/445 rows currently have `inicio_tipo='data-publicacao'`, so criterion 3's flagged label has no live instance in production right now, and a non-fabricated fixture (per #167's own 'sem fabricar conteúdo' resume condition) can't be built from real data until some future parse actually asserts an explicit `<inicio>`. This doesn't close #167 (the risk is dormant, not eliminated), but confirms there's nothing to observe/fix yet even independent of the CORS blocker. Resume path unchanged: either resolve browser-read-cors-integrity (proxy or fixed IA CORS) so a real `/lei/?id=...` capture becomes possible, or wait for real corpus growth to produce a genuine `data-publicacao` row to build a non-fabricated fixture from — only then add whatever disclosure copy is still warranted."
