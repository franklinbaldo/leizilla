...
timestamp: 2026-09-25T10:35:00+00:00
...
fronts:
  ...
  - id: "ingestion-resilience"
    ...
    next_action: "New session 2026-09-25T09:2x-10:0xZ: merged PR #234 (the discover-shared/harvest-parallel split fixing #141's real root cause) and PR #235. Found the OLD run 36082100347 was still executing the PRE-fix workflow (its `lc` matrix job's Discover step had been running 5+ hours alone, 5/8 tipos still queued) and, since it also held the `discover-harvest` concurrency singleton, was blocking any new dispatch from testing the fix — cancelled it and dispatched a fresh run (36118095901, `tipo=\"\"`, exercising the new `discover-shared` → artifact → `harvest-parallel` path) on the fixed workflow. That run's `discover-shared` job was STILL in its single `Discover` step at session end (~50min in, not yet timed out or stuck — consistent with the ~1.5h per-discovery pace seen before), so the fix is not yet verified end-to-end; check run 36118095901's final conclusion and total wall-clock next session — this is the answer to kr-ro-recurring-cycle-health's standing question. #149's scope-split fix still needs a second SCHEDULED (not dispatched) clean run before closing. #136/#140 stay deferred to pipeline-convergence. Also this session: issue #233 (found last session — `fetch-all-parsed` can silently miss items the same workflow run just parsed, due to Internet Archive search-index lag) is now FIXED and MERGED (PR #239, deterministic id hand-off between parse jobs and the etl job via a build artifact, not a retry/sleep) — see dataset-release-integrity for detail; issue #233 auto-closed."
  ...
  - id: "release-boundary-validation"
    ...
    issues: [118, 201, 229]
    evidence_prs: [192, 193, 198, 206, 213, 215, 216, 218, 221, 222, 228, 230, 239]
    next_action: "Issue #233 CLOSED (PR #239, merged): `fetch-all-parsed` could run before the Internet Archive's own search index had caught up with a same-run upload, silently publishing `-latest` without items the very same workflow run had just parsed (the row-count floor guard didn't gate this — the incomplete release still had more rows than the previous floor). Fix is a deterministic direct id hand-off, not a retry/sleep: `parse-all --uploaded-ids-out` records uploaded ia_ids → workflow artifact → `etl` job merges all parse jobs' artifacts → `fetch-all-parsed --extra-ids-file` unions them with the search result before downloading (fetch-by-id isn't subject to the search lag). See dataset-release-integrity for full detail, tests and CI evidence. See legal-semantic-integrity's kr-release-validation-gated for the rest of what's merged. `-latest` published for the first time this session (run 36094205340, 209 rows). Item 4 fully resolved (PR #221). Item 13's OCR-derivative gap still has no djvu.txt as of 2026-09-25T06:45Z; the raw item's pending-tasks flag is still set, so it may resolve without any Leizilla-side fix — see dataset-release-integrity for the forensics and revised next steps. PR #228 (merged) closed a distinct gate gap: <inicio> without tipo could silently publish as data-publicacao; issue #229 tracks the follow-up (the schema-mandated <inicio>/<fonte> evidence is parsed for XSD validation but never reaches the Parquet)."
  ...
  - id: "federal-expansion-q1-2027"
    ...
    next_action: "#175, #157, #118 and #195 (urn_lex gate, PR #206) are all resolved. #196 is resolved — `-latest` published for the first time this session (run 36094205340, 209 rows). #201 is down to item 13 alone, confirmed a genuine external IA OCR-derivative gap (not a Leizilla code defect); re-checked 2026-09-25T10:2xZ (new session) — `archive.org/metadata/leizilla_ro_casacivil_lei_0001-1000` still reports `pending_tasks: true` and still has no `lei-00013_85b0957e_djvu.txt` among its files, no change since the prior 05:45Z/07:3xZ readings. Re-evaluate whether this single-item residual gap is acceptable to export alongside federal expansion, or whether it should block until resolved — a judgment call for the next session/maintainer, not yet made."
    blockers:
