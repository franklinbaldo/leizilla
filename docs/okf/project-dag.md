...
fronts:
  ...
  - id: "ro-coverage-q4-2026"
    ...
    key_results:
      ...
      - id: "kr-ro-recurring-cycle-health"
        ...
        next_action: "#149: verified once (run 36082098747) — close after a second clean run. #141: run 36082100347 is genuinely progressing (not stuck); let it finish (or check a future scheduled run) and measure total wall-clock for a full 8-tipo cycle before deciding whether max-parallel:1 alone is sufficient or whether per-scheduled-run tipo scoping is also needed. #136/#140 close only once rondonia_crawler.yml is deprecated (pipeline-convergence)."
  ...
  - id: "ingestion-resilience"
    ...
    next_action: "Issue #121 (the transient-failure-becomes-permanent-loss defect) is fixed and merged (PR #190). #149's scope-split fix (PR #207) is verified working (wayback-save.yml run 36082098747, all 8 matrix jobs green). #141's discover-harvest.yml (run 36082100347) is confirmed genuinely progressing, not stuck, but full-cycle wall-clock at the new serialized pace is unmeasured — see kr-ro-recurring-cycle-health. #136/#140 stay deferred to pipeline-convergence."
  ...
  - id: "browser-read-cors-integrity"
    ...
    issues: [223]
    evidence_prs: [224]
    next_action: "Confirmed twice live against archive.org production (not just a local dev server): (1) a prior session's Playwright run against a local astro dev server pointed at the real published dataset, and (2) this session's direct curl against both the archive.org redirect and the ia601806.us.archive.org storage node for the real 76846-byte leizilla-dataset-ro-v0/versoes.parquet — 206 Partial Content with no access-control-allow-origin, while dataset_meta.json on the same item has it. PR #224 (merged) is a mitigation, not a fix: probeDatasetAccess() (web/src/lib/db.ts) now distinguishes this permanent CORS block from a generic transient failure, so DatasetUnavailable.svelte stops telling users 'try again' for something that will never succeed. The actual fix needs a proxy (porting causaganha's Cloudflare Worker, issue #1482/PR #1521 there) — an infrastructure decision beyond IA/GitHub that deserves an ADR given CLAUDE.md's infrastructure-minimal principle, and needs Cloudflare (or equivalent) credentials no session has had yet. Also still open: confirm this reproduces against the actually-deployed franklinbaldo.github.io bundle's exact configured PUBLIC_PARQUET_URL (not just archive.org generically), and consider reporting the CORS gap to Internet Archive directly since it may be fixable upstream with no proxy needed at all. This is an external/infrastructure blocker, not a Leizilla code defect — no further code-level PR should attempt to work around it beyond the classification already merged."
