...
| **M14.4** — Segmentador regex baseline + eval vs gold | 🟢 done | — | `segmenter.py` (Pattern B) + CLI `opf-regex-eval` + `evaluate_against_gold` (exact/overlap P/R/F1 por categoria). Regras complexas: splitter de sentença ciente de abreviações/números, filtro de verbo operativo na revogação (exclui notas `(Revogado pela…)`, precision 0.33→1.00 em compilados), strip de marcador líder em cláusulas, guard de contexto à direita (`§ N do…`, `(VETADO)`). v0 held-out: exact micro-F1 **0.91** / overlap **0.99** (art/inc/ali/ementa/vigência/revogação exact 1.00). Residual = xref § (~0.76) e textos compilados → território do OPF. 18 testes. |
